 ...
 */
@FunctionalInterface
public interface FailureHandler extends BiConsumer<CommandContext, FailureReason> {
    
    /**
     * Performs the operation on the given context with a given failure reason.
     *
     ...
     * @return The composed FailureHandler.
     * @see BiConsumer#andThen(BiConsumer)
     */
    default FailureHandler andThen( FailureHandler after ) {
        
        return ( context, reason ) -> {
            
            this.accept( context, reason );
            after.accept( context, reason );

...
public class CommandHandler implements IListener<MessageReceivedEvent> {
    ...
    /**
     * Regex that matches a string that starts with a quote and, at some point
     * before the end, has another quote that either is followed by one or more
     * whitespaces or is the last character in the string.
     * <p>
     * eg, if the string is an argument string, matches it if the first argument in
     * the string is between quotes (starts with a quote and end with a quote,
     * followed by a whitespace or the end of the string).
     */
    ...
    /**
     * When a message is received, parses it to determine if it has the signature of
     * a registered command.<br>
     * If it does, checks if the command should be executed (based on context and
     * the command's settings) and executes it if so.
     * 
     * @param event
     *            Event fired by a message being received.
     */
    @Override
     ...
     * <p>
     * An "argument" is considered as text preceded by either a space or the start
     * of the string, and followed by either a space or the end of the string. Text
     * between two double-quotes (where the first quote is preceded by either a
     * space or the start of the string, and the second is followed by either a
     * space or the end of the string) is considered a single argument, not
     * including the quotes (it may have spaces within it (or other double-quotes,
     * as long as they are followed by non-space characters).
     *
     * @param argString
     *            The argument string to be split. Is assumed to not have any
    ...
    /**
     * Obtains the subcommand chain of the given command with the given
     * arguments.<br>
     ...
     *            Main command called.
     * @param args
     *            Arguments passed in to the command.
     ...
     * That is, gets the commands that should be executed according to the
     * {@link ICommand#executeParent()} setting of the commands.
     * <p>
     * After this method returns, the command on top of the stack is the first one
     * that should be executed, with the other commands in the stack also being
     * retrieved in the order that they should be executed.
     *
     * @param executionChain
     *            Where the commands to be executed should be added.
     * @param commands
     *            The commands that were triggered.
     */
    ...
    /**
     * Rebuilds the signature of a command given the split args in the message and
     * the amount of commands that were parsed from it.
     *
     * @param command
     *            The command from the message received.
     * @param args
     *            The args from the message received.
     * @param subCommandAmount
     *            How many subcommands are in the args.
    ...
    /**
     * Buils a trace message for a command that includes the command name, who
     * called it, and where it was called from.
     *
     * @param command
     *            Command called.
     * @param event
